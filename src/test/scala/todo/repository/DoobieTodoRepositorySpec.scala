 ...
 *
 * Start one with `docker compose up -d postgres-test`, then run
 * `TODO_TEST_DB=1 sbt test`. Without that env var the suite is skipped so a
 * plain `sbt test` stays hermetic.
 */
class DoobieTodoRepositorySpec extends CatsEffectSuite:
  ...
  test("filters by status, due date and free text") {
    withRepository { repository =>
      val base = Instant.parse("2026-01-01T00:00:00Z")
      for
        _ <- repository.insert(todo("buy milk", TodoStatus.Pending, base, Some(base.plusSeconds(60)), Some("corner shop")))
  ...
  test("replace updates in place and reports a missing row") {
    withRepository { repository =>
      ...
      val edited   = original.copy(title = "edited", status = TodoStatus.Done, dueAt = Some(Instant.parse("2026-05-05T05:05:05Z")))
      for
  ...
  test("schema bootstrap is idempotent") {
    withRepository { repository =>
      ...
      transactor.use { xa =>
        Migrator.run[IO](xa) *> Migrator.run[IO](xa) *> repository.count(TodoFilter.all).map(total => assertEquals(total, 0L))
      }
    }
  }
